    ...
    >
      <div className="relative grid aspect-[4/3.45] min-h-[165px] place-items-center overflow-hidden bg-white min-[420px]:min-h-[205px] sm:min-h-[245px] lg:min-h-[275px]">
        ...
        <div className="absolute left-3 top-3 z-10 flex flex-col gap-2">
          {product.bestSeller ? (
            <span className="rounded-full bg-forest px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-forest-foreground">
              Best seller
            </span>
          ) : null}
          {product.newArrival ? (
            <span className="rounded-full bg-saffron px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white">
              New
            </span>
          ) : null}
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col p-4 sm:p-5">
        ...
        <Link
          to={`/product/${product.slug}`}
          className="mt-2 break-words font-display text-base font-bold leading-snug text-foreground hover:text-accent sm:text-lg"
        >
          {product.name}
        </Link>
        {!compact ? (
        ...
        <div className="mt-auto flex flex-col gap-3 pt-4 min-[380px]:flex-row min-[380px]:items-end min-[380px]:justify-between">
          ...
          <div className="shrink-0 text-left min-[380px]:text-right">
            <p className="flex items-center gap-1 text-xs font-semibold text-foreground min-[380px]:justify-end">
              <Star className="h-3.5 w-3.5 fill-saffron text-saffron" /> {product.rating}
            </p>
            <button
              onClick={add}
              className="mt-2 inline-flex min-h-9 items-center gap-1.5 rounded-full bg-primary px-3 py-2 text-xs font-bold text-primary-foreground transition hover:-translate-y-0.5 hover:bg-forest"
            >

...
const PRODUCT_IMAGE_OVERRIDES = new Map(PRODUCTS.map((item) => [item.slug, item.image]));

function isSourceAsset(value?: string) {
  return /^\/?src\/assets\//.test(String(value || ""));
}

function productImage(raw: any, imageOverride?: string) {
  const image = raw.image || raw.mainImage || "";
  if (!image || isSourceAsset(image)) return imageOverride || image;
  return image;
}
...
function normalizeProduct(raw: any): Product {
  ...
  return {
    ...
    image: productImage(raw, imageOverride),

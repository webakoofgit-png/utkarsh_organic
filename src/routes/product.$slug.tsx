...
import { getProduct, inr, priceFor, PRODUCTS, WEIGHTS, type Weight } from "@/lib/products";
...
export default function ProductDetailPage() {
  ...
  const [weight, setWeight] = useState<Weight>("100g");
    ...
    <main className="pt-16 lg:pt-20">
      ...
      <section className="container-x grid gap-10 pb-16 pt-4 lg:grid-cols-2 lg:pb-24">
        ...
        <div className="lg:py-4"><p className="eyebrow">{product.category} powder</p><div className="mt-3 flex items-start justify-between gap-4"><h1 className="font-display text-4xl font-extrabold leading-tight sm:text-5xl">{product.name}</h1><button onClick={() => toggleWishlist(product.slug)} aria-label="Add product to wishlist" className={`grid h-11 w-11 shrink-0 place-items-center rounded-full border border-border ${saved ? "bg-secondary text-destructive" : "text-foreground hover:bg-secondary"}`}><Heart className="h-5 w-5" fill={saved ? "currentColor" : "none"} /></button></div><div className="mt-4 flex flex-wrap items-center gap-4"><span className="flex items-center gap-1 text-sm font-bold"><Star className="h-4 w-4 fill-saffron text-saffron" /> {product.rating} <span className="font-normal text-muted-foreground">({product.reviews} reviews)</span></span><span className="h-4 border-l border-border" /><span className="text-sm font-semibold text-accent">{product.inStock ? "In stock & ready to ship" : "Currently unavailable"}</span></div><p className="mt-6 max-w-xl leading-relaxed text-muted-foreground">{product.description}</p>
          ...
          <div className="mt-7 flex items-end gap-4"><div><p className="font-display text-3xl font-extrabold">{inr(prices.price)}</p><p className="mt-1 text-sm text-muted-foreground"><span className="line-through">{inr(prices.mrp)}</span> &middot; Save {inr(prices.mrp - prices.price)}</p></div></div>
          ...
          <div className="mt-8 grid gap-3 border-t border-border pt-6 sm:grid-cols-3">{[{ icon: Truck, text: "Dispatch in 24 hours" }, { icon: ShieldCheck, text: "Secure payments" }, { icon: LeafIcon, text: "Clean ingredients" }].map(({ icon: Icon, text }) => <p key={text} className="flex items-center gap-2 text-xs font-semibold text-muted-foreground"><Icon className="h-4 w-4 text-accent" /> {text}</p>)}</div>
        </div>
      </section>
      <section className="bg-beige/45 py-16 lg:py-20"><div className="container-x grid gap-10 lg:grid-cols-3"><div className="lg:col-span-2"><h2 className="font-display text-3xl font-extrabold">Why you will reach for it</h2><div className="mt-6 grid gap-3 sm:grid-cols-2">{product.benefits.map((benefit) => <p key={benefit} className="flex gap-3 rounded-2xl bg-background p-4 text-sm leading-relaxed"><Check className="mt-0.5 h-4 w-4 shrink-0 text-accent" /> {benefit}</p>)}</div></div><aside className="rounded-3xl bg-forest p-7 text-forest-foreground"><h3 className="font-display text-xl font-bold">The essentials</h3><p className="mt-5 text-xs font-bold uppercase tracking-widest text-accent">Ingredients</p><p className="mt-2 text-sm leading-relaxed text-forest-foreground/80">{product.ingredients}</p><p className="mt-5 text-xs font-bold uppercase tracking-widest text-accent">Storage</p><p className="mt-2 text-sm leading-relaxed text-forest-foreground/80">{product.storage}</p></aside></div></section>
      <section className="container-x grid gap-10 py-16 lg:grid-cols-[0.85fr_1.15fr] lg:py-24"><div><h2 className="font-display text-3xl font-extrabold">How to use it</h2><ol className="mt-6 space-y-4">{product.usage.map((tip, index) => <li key={tip} className="flex gap-4"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-secondary text-xs font-bold text-primary">{index + 1}</span><p className="pt-1 text-sm leading-relaxed text-muted-foreground">{tip}</p></li>)}</ol></div><div className="rounded-3xl border border-border p-6 sm:p-8"><h2 className="font-display text-2xl font-extrabold">Nutrition information</h2><p className="mt-2 text-sm text-muted-foreground">Typical values per 100g</p><div className="mt-5 divide-y divide-border">{product.nutrition.map((item) => <div key={item.label} className="flex justify-between gap-4 py-3 text-sm"><span>{item.label}</span><strong>{item.value}</strong></div>)}</div></div></section>

...
export default function ProductDetailPage() {
  ...
  if (!product) {
    return (
      <main className="container-x pb-20 pt-32 text-center">
        <h1 className="font-display text-3xl font-extrabold sm:text-4xl">Product Not Found</h1>
        <p className="mt-2 text-muted-foreground">
          The organic product you are looking for does not exist.
        </p>
        <Link
          to="/products"
  ...
  const priceLabel =
    prices.mrp > prices.price ? `${inr(prices.price)} - ${inr(prices.mrp)}` : inr(prices.price);
    ...
    <main className="pt-16 lg:pt-20">
        ...
        <div className="min-w-0 lg:py-4">
          ...
          <div className="mt-3 flex items-start justify-between gap-3 sm:gap-4">
            ...
            <button
              ...
              aria-label={saved ? "Remove product from wishlist" : "Add product to wishlist"}
              className={`grid h-11 w-11 shrink-0 place-items-center rounded-full border border-border ${
                saved ? "bg-secondary text-destructive" : "text-foreground hover:bg-secondary"
              }`}
            >
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 sm:gap-4">
            <span className="flex items-center gap-1 text-sm font-bold">
              <Star className="h-4 w-4 shrink-0 fill-saffron text-saffron" /> {product.rating}
            ...
            <span className="text-sm font-semibold text-accent">
              {product.inStock ? "Available for quote" : "Currently unavailable"}
            </span>
            <span className="hidden h-4 border-l border-border sm:block" />
            <span className="text-sm font-semibold text-muted-foreground">MOQ {product.moq}</span>
          </div>
          ...
          <div className="mt-7">
            <p className="text-sm font-bold">Choose pack size</p>
          ...
          <div className="mt-7">
            <p className="font-display text-2xl font-extrabold sm:text-3xl">{priceLabel}</p>
            <p className="mt-1 text-sm text-muted-foreground">
              {weight} selected &middot; Official listing: {product.priceLabel}
            </p>
          </div>

          <div className="mt-7 grid gap-3 sm:grid-cols-[auto_minmax(0,1fr)_auto]">
            <div className="flex h-12 w-max items-center rounded-full border border-border bg-background">
              <button
            ...
            <button
              ...
              onClick={add}
              className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-primary px-6 text-sm font-bold text-primary-foreground transition hover:bg-forest disabled:opacity-50"
            >
            ...
            <button
              ...
              className="h-12 w-full rounded-full bg-accent px-5 text-sm font-bold text-accent-foreground sm:w-auto"
            >
              Buy now
            </button>
          </div>

          <div className="mt-8 grid gap-3 border-t border-border pt-6 min-[480px]:grid-cols-3">
            {[
              { icon: Truck, text: "Pan India supply" },
      ...
      <section className="bg-beige/45 py-16 lg:py-20">
        <div className="container-x grid gap-10 lg:grid-cols-3">
          ...
          <aside className="min-w-0 rounded-3xl bg-forest p-6 text-forest-foreground sm:p-7">
            <h3 className="font-display text-xl font-bold">The essentials</h3>
            ...
            <p className="mt-5 text-xs font-bold uppercase tracking-widest text-accent">Storage</p>
            <p className="mt-2 text-sm leading-relaxed text-forest-foreground/80">
              {product.storage}
            </p>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-5 inline-block text-xs font-bold text-accent underline underline-offset-4"
            >
              View official source listing
            </a>
          </aside>
        </div>
      </section>
          ...
          <ol className="mt-6 space-y-4">
            {product.usage.map((tip, index) => (
              <li key={tip} className="flex gap-4">
                ...
                <p className="pt-1 text-sm leading-relaxed text-muted-foreground">{tip}</p>
              </li>
            ))}
          </ol>
        </div>

        <div className="min-w-0 rounded-3xl border border-border p-5 sm:p-8">
          <h2 className="font-display text-2xl font-extrabold">Product specifications</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Based on the official Utkarsh Organic Farm listing
          </p>
          <div className="mt-5 divide-y divide-border">
            {product.specs.map((item) => (
              ...
              >
                ...
                <strong className="min-w-0 break-words sm:text-right">{item.value}</strong>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="container-x">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-3 font-display text-3xl font-extrabold">You may also like</h2>
              </div>
              <Link to="/products" className="text-sm font-bold text-primary underline underline-offset-4">
                All products
              </Link>
            </div>
...
function LeafIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={className}>
      <path d="M20 3C11 3 4 8 4 16c0 2 1 4 3 5 8 0 13-7 13-18Z" />
